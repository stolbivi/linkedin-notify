...
import {moveCard, setActiveTab, setKanbanData, updateCardIdByOptimisticId} from '../../../../../store/kanban.slice';
// @ts-ignore
import stylesheet from './styles.scss';
import {MessagesV2} from "@stolbivi/pirojok";
import {NoteExtended, VERBOSE} from "../../../../../global";
import {getAuthorStages, getCustomStages, setStage,} from "../../../../../actions";
import {Loader} from "../../../../../components/Loader";
import ListView from "../ListView";
import {CompleteEnabled, DataWrapper, selectNotesAll} from "../../../../../store/LocalStore";
import {shallowEqual, useSelector} from "react-redux";
import {appendNoteAction, removeNoteByStageTo} from '../../../../../store/NotesAllReducer';
import {StageLabels} from '../../../../notes/StageSwitch';
...
const KanbanBoard: React.FC<any> = () => {
    ...
    const onDragEnd = (result: DropResult) => {
        ...
        if (activeButton === IStatus.GROUPS) {
            messages.request(setStage({
                id: selectedCard.profileId,
                stage: Object.values(ICategory).indexOf(destination.droppableId),
                stageFrom: Object.values(ICategory).indexOf(source.droppableId),
                stageText: destination.droppableId || undefined,
                    ...
                    (completed && winReady) ? (
                        <>
                            <div className="button-container">
                                <button className={`button ${activeButton === IStatus.AVAILABILITY ? 'active' : ''}`}
                                        onClick={() => handleClick(IStatus.AVAILABILITY)}>
                                    Availability
                                </button>
                                <button className={`button ${activeButton === IStatus.STATUS ? 'active' : ''}`}
                                        onClick={() => handleClick(IStatus.STATUS)}>
                                    Status
                                </button>
                                <button className={`button ${activeButton === IStatus.TYPE ? 'active' : ''}`}
                                        onClick={() => handleClick(IStatus.TYPE)}>
                                    Type
                                </button>
                                <button className={`button ${activeButton === IStatus.GEOGRAPHY ? 'active' : ''}`}
                                        onClick={() => handleClick(IStatus.GEOGRAPHY)}>
                                    Geography
                                </button>
                                <button className={`button ${activeButton === IStatus.GROUPS ? 'active' : ''}`}
                                        onClick={() => handleClick(IStatus.GROUPS)}>
                                    Groups
                                </button>
                                    ...
                                    <div>
                                        <svg className="icon-color" width="16" height="16" viewBox="0 0 16 16"
                                            ...
                                            <path
                                                ...
                                                fill="#585858"/>
                                            <path
                                                d="M14 6.83334H2C1.72667 6.83334 1.5 6.60668 1.5 6.33334C1.5 6.06001 1.72667 5.83334 2 5.83334H14C14.2733 5.83334 14.5 6.06001 14.5 6.33334C14.5 6.60668 14.2733 6.83334 14 6.83334Z"
                                                fill="#585858"/>
                                            <path
                                                d="M14 10.1667H2C1.72667 10.1667 1.5 9.93999 1.5 9.66666C1.5 9.39332 1.72667 9.16666 2 9.16666H14C14.2733 9.16666 14.5 9.39332 14.5 9.66666C14.5 9.93999 14.2733 10.1667 14 10.1667Z"
                                                fill="#585858"/>
                                            <path
                                        ...
                                        <button id="kanban-list-view-btn"
                                                className={`btn list-btn ${listView ? "" : "opacity-60"}`}
                                                onClick={(event) => listViewClickHandler(event)}>
                                            List View
                                        </button>
                                    </div>
                                    <div>
